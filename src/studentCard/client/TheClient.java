...
public class TheClient {
	...
	byte[] paddingCheck(byte[] buf) {
		...
		// Else no padding so we delete 8 bytes of padding
		else{
	...
	void cipherFileByCard() {
		...
		try {
			...
			byte[] buffer = new byte[(length%8==1)?length+16-length%8 : length+8-length%8]; // Buffer for the cipher
			byte[] tmp = new byte[MAX_SIZE];
			byte[] resp = new byte[MAX_SIZE];

			dataIS.readFully(buf); // Read all the main buffer data
			int i = 0;
